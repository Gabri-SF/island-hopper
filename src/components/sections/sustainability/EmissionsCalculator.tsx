...
const reference = {
  airplane: { label: "Avião Convencional (por pax)", value: "18–20", perFlight: "~126–140" },
  ferry: { label: "Ferry Rápido (por pax)", value: "2–4", perFlight: "~14–28" },
};
...
export default function EmissionsCalculator() {
  ...
  const c = colorMap[scenario.color as "emerald" | "amber"];
    ...
    <div className="flex flex-col gap-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <h3 className="text-2xl font-black text-zinc-100">
              {scenario.total} <span className="text-base font-bold text-zinc-400">kg CO₂-eq / voo</span>
            </h3>
            <p className="text-xs text-zinc-500 mt-1">{scenario.note}</p>
          </div>
          <div className="text-right">
            <span className="text-xs text-zinc-600 block">Por passageiro</span>
      ...
      <div>
        <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-widest mb-4">
          Comparação com Modos Convencionais (por voo, 7 pax)
        </h3>
        <div className="flex flex-col gap-3">
          <div className="p-4 rounded-xl bg-zinc-900/40 border border-zinc-800 flex items-center justify-between">
            <span className="text-sm font-semibold text-zinc-300">✈ Avião Convencional</span>
            <div className="text-right">
              <span className="text-sm font-bold text-red-400 font-mono">
                {reference.airplane.perFlight} kg CO₂
              </span>
              <span className="block text-[10px] text-zinc-500">{reference.airplane.value} kg/pax</span>
            </div>
          </div>
          <div className="p-4 rounded-xl bg-zinc-900/40 border border-zinc-800 flex items-center justify-between">
            <span className="text-sm font-semibold text-zinc-300">⛴ Ferry Rápido</span>
            <div className="text-right">
              <span className="text-sm font-bold text-zinc-400 font-mono">
                {reference.ferry.perFlight} kg CO₂
              </span>
              <span className="block text-[10px] text-zinc-500">{reference.ferry.value} kg/pax</span>
            </div>
          </div>
          <div className={`p-4 rounded-xl border ${c.border} ${c.bg} flex items-center justify-between`}>
            ...
            <div className="text-right">
              ...
              <span className="block text-[10px] text-zinc-500">
                {(scenario.total / 7).toFixed(2)} kg/pax · Zero emissões diretas
              </span>
            </div>
          </div>
        </div>
