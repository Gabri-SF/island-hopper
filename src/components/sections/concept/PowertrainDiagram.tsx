...
export default function PowertrainDiagram() {
  ...
  const dict = dictionaries[language]?.powertrainDiagram || {
    ...
    caption: "Vista superior esquemática — powertrain do Island Hopper",
    legendTitle: "LEGENDA",
    legendLabel: "Estruturas",
    labels: {
      avionica: "AVIÓNICA",
      bmsFcs: "BMS · FCS",
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/20 overflow-hidden p-4">
      <svg
        ...
        aria-label="Powertrain configuration layout model visualization map"
      >
        {/* ── DIRECTION LABELS ── */}
        <text x={cx} y="28" textAnchor="middle" fontSize="8.5" fill="#3f3f46" letterSpacing="3" fontWeight="600">{dict.proa}</text>
        <text x={cx} y="642" textAnchor="middle" fontSize="8.5" fill="#3f3f46" letterSpacing="3" fontWeight="600">{dict.cauda}</text>
        ...
        <text x="370" y="150" fontSize="7.5" fill="#3f3f46">{dict.canard}</text>
        ...
        <text x="610" y="330" textAnchor="middle" fontSize="7.5" fill="#3f3f46">{dict.asaPrincipal}</text>
        ...
        <text x={cx} y="582" textAnchor="middle" fontSize="7.5" fill="#3f3f46">{dict.ttail}</text>
        ...
        <text x={cx} y="80" textAnchor="middle" fontSize="9.5" fill="#86efac" fontWeight="700">{dict.labels.avionica}</text>
        <text x={cx} y="93" textAnchor="middle" fontSize="8" fill="#4ade80">{dict.labels.bmsFcs}</text>
        <text x={cx} y="105" textAnchor="middle" fontSize="7" fill="#22c55e" opacity="0.8">{dict.labels.controloVoo}</text>

        {/* Li-S Battery */}
        <rect x="422" y="208" width="54" height="56" rx="3" fill="#1e3a5f" stroke="#3b82f6" strokeWidth="1.5" />
        <text x="449" y="226" textAnchor="middle" fontSize="9" fill="#93c5fd" fontWeight="700">{dict.labels.bateria}</text>
        <text x="449" y="238" textAnchor="middle" fontSize="8" fill="#7dd3fc">{dict.labels.lis}</text>
        <text x="449" y="251" textAnchor="middle" fontSize="7.5" fill="#60a5fa">{dict.labels.massaBat}</text>
        ...
        <text x="511" y="238" textAnchor="middle" fontSize="8" fill="#7dd3fc">{dict.labels.tanque}</text>
        <text x="511" y="251" textAnchor="middle" fontSize="7.5" fill="#38bdf8">{dict.labels.massaTanque}</text>

        {/* Thermal Management Grid Box */}
        <rect x="420" y="272" width="120" height="86" rx="4" fill="#0d3330" stroke="#14b8a6" strokeWidth="1" strokeDasharray="4,3" opacity="0.75" />
        <text x="430" y="284" fontSize="7" fill="#2dd4bf" opacity="0.7">{dict.labels.gestaoTermica}</text>

        {/* PEMFC */}
        <rect x="430" y="294" width="100" height="54" rx="3" fill="#115e59" stroke="#0d9488" strokeWidth="1.5" />
        <text x={cx} y="315" textAnchor="middle" fontSize="9.5" fill="#5eead4" fontWeight="700">{dict.labels.pemfc}</text>
        <text x={cx} y="328" textAnchor="middle" fontSize="8" fill="#14b8a6">{dict.labels.metricaPemfc}</text>
        ...
        <text x={cx} y="387" textAnchor="middle" fontSize="9" fill="#fb923c" fontWeight="700">{dict.labels.barramento}</text>

        {/* HV Converters */}
        <rect x="422" y="408" width="54" height="42" rx="3" fill="#2c1303" stroke="#f97316" strokeWidth="1" />
        <text x="449" y="426" textAnchor="middle" fontSize="8" fill="#fdba74" fontWeight="600">{dict.labels.conv}</text>
        <text x="449" y="438" textAnchor="middle" fontSize="7.5" fill="#f97316">{dict.labels.hvx2}</text>

        {/* DC/DC Converter */}
        <rect x="484" y="408" width="54" height="42" rx="3" fill="#2c1303" stroke="#f97316" strokeWidth="1" />
        <text x="511" y="426" textAnchor="middle" fontSize="8" fill="#fdba74" fontWeight="600">{dict.labels.conv}</text>
        <text x="511" y="438" textAnchor="middle" fontSize="7.5" fill="#f97316">{dict.labels.dcdc}</text>

        {/* Flight Computer */}
        <rect x="424" y="130" width="112" height="48" rx="3" fill="#1e1033" stroke="#a855f7" strokeWidth="1.5" />
        <text x={cx} y="150" textAnchor="middle" fontSize="9" fill="#d8b4fe" fontWeight="700">{dict.labels.computador}</text>
        <text x={cx} y="163" textAnchor="middle" fontSize="7.5" fill="#a78bfa">{dict.labels.fmsGnss}</text>
        ...
        <text x="650" y="595" textAnchor="middle" fontSize="8.5" fill="#3f3f46">{dict.envergadura}</text>
        ...
        {/* ── BORDO LABELS ── */}
        <text x="120" y="392" textAnchor="middle" fontSize="8" fill="#3f3f46" fontWeight="600">{dict.bordoEsquerdo}</text>
        <text x="840" y="392" textAnchor="middle" fontSize="8" fill="#3f3f46" fontWeight="600">{dict.bordoDireito}</text>

        {/* ── LEGEND ── */}
        <g transform="translate(28, 30)">
          <rect x="0" y="0" width="178" height="166" rx="6" fill="#0a0a0b" stroke="#27272a" strokeWidth="1" />
          <text x="89" y="16" textAnchor="middle" fontSize="7.5" fill="#52525b" fontWeight="700" letterSpacing="2">{dict.legendTitle}</text>
          {(dict.items as { color: string; label: string }[]).map((item, i) => (
          ...
          <text x="10" y="160" fontSize="7" fill="#52525b">{dict.legendLabel}</text>
        </g>
      </svg>
